...
const Target = async function(username){
	...
	try{
		...
		if (json.entry_data.ProfilePage[0].graphql.user.is_private) {
			...
			const Following = json.entry_data.ProfilePage[0].graphql.user.edge_followed_by.count;
			return Promise.resolve({id,Following});
		}
	} catch (err){
		return Promise.reject(err);
	}
...
const CommentAndLike = async function(session, accountId, text){
	...
	if (result.length > 0) {
		...
		const printComment = Comment ? chalk`{green Comment}` : chalk`{red Commentr}`;
		const printLike = Like ? chalk`{green Like}` : chalk`{red Liker}`;
...
const Following = async function(session, id){
	...
	try{
		...
		do {
			...
			const getFollowing = await feed.get();
			await Promise.all(getFollowing.map(async(akun) => {
				Following.push(akun.id);
			}))
...
const Excute = async function(User, TargetUsername, Text, Sleep){
	try {
		...
		console.log(chalk`{green  | Login Succsess, try to get Following Target ....}`)
		const getTarget = await Target(TargetUsername);
		console.log(chalk`{green  | ${TargetUsername}[${getTarget.id}] Following: ${getTarget.Following}}`)
		const getFollowing = await Following(doLogin.session, doLogin.account.id)
		console.log(chalk`{cyan  | Try to Follow, Comment, and Like Following Target ... \n}`)
		const Targetfeed = new Client.Feed.AccountFollowing(doLogin.session, getTarget.id);
		var TargetCursor;
		do {
	      ...
	      timeNow = `${timeNow.getHours()}:${timeNow.getMinutes()}:${timeNow.getSeconds()}`
				await Promise.all(TargetResult[i].map(async(akun) => {
					if (!getFollowing.includes(akun.id) && akun.params.isPrivate === false) {
